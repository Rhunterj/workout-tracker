...
import { createWorkout, getCurrentExercises, saveCurrentWorkout } from "@/lib/workout-service"
...
export default function NewWorkoutPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Validate exercises
    const invalidExercises = exercises.filter((ex) => !ex.name || ex.sets.length === 0)
    ...
    <main className="container max-w-md mx-auto p-4 pb-24">
            ...
            <div className="grid gap-2">
              ...
              <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" className="w-full justify-start text-left font-normal" id="workout-date">
        ...
        {exercises.length === 0 ? (
          ...
          <div className="space-y-4">
            {exercises.map((exercise, index) => (
              <ExerciseItem key={exercise.id} exercise={exercise} onRemove={handleRemoveExercise} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-16 left-0 right-0 flex justify-center">
        <div className="w-full max-w-md px-4 flex gap-2">
          <Button variant="outline" size="lg" className="w-1/3" onClick={() => router.push("/")}>
            Cancel
          </Button>
          <Button size="lg" className="w-2/3 gap-2" onClick={handleSubmit} disabled={isSubmitting}>
